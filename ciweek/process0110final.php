...
$mysql_password="";//密码
$mysql_database="ciweek_com";//数据库名
/*$mysql_server_name="localhost";//服务器
$mysql_username="root";//数据库用户名
$mysql_password="";//密码
...
print_r($topiclink[1][0]);///article/2016/1107/A20161107574337.shtml

//匹配topic的图片
preg_match_all('/<dl class="toppic">[\s\S].*?<dt[^>]*.<a href=".*?<img src="(.*?)".*<\/a><\/dt>/is',$str,$topicarr);
print_r($topicarr);die;
...
//print_r($arr2[1]);//列表的图片数组
foreach ($link_arr[1] as $k => $v){
  //print_r($link_arr[1]);//列表的链接数组
  //print_r($v);//单个列表的链接/article/2016/1107/A20161107574336.shtml

    $img = $arr2[1][$k];//$k单个列表的链接下标

    $thumbarr['http://www.ciweek.com'.$v] = $img; 
    
    //print_r('http://www.ciweek.com'.$v);
  }
}
//print_r(count($arr2[1]));
//print_r($arr2);die;
//print_r($str);
//print_r($thumbarr);die;//列表的链接=>列表的图片数组

//print_r(array_keys($thumbarr));die;
//var_dump($thumbarr['pages/A20151112568595.shtml']);
//打印出了string(53) "http://images.enet.com.cn/i/2016/1021/051800897.jpg">"
//print_r($topiclink[1][0]);die;// /article/2016/1107/A20161107574337.shtml
//var_dump($topicarr[1][0]);die;//http://images.enet.com.cn/2016/1019/11/8476108.jpg 
////准备要删除的，因为从phpcms后台直接录入
$thumbarr['http://www.ciweek.com'.$topiclink[1][0]] = $topicarr[1][0];
//print_r('http://www.ciweek.com'.$topiclink[1][0]);//http://www.ciweek.com/article/2016/1107/A20161107574337.shtml
//print_r($topicarr[1][0]);die;//http://images.enet.com.cn/2016/1107/85/5024239.jpg
//print_r($thumbarr['pages'.substr($topiclink[1][0],strrpos($topiclink[1][0],'/'))]);die;//是topic的图片地址
...
foreach ($thumbarr as $key2 => $value2) {
 //var_dump($value2);die;//http://images.enet.com.cn/2016/1107/80/5019169.jpg
    phpQuery::newDocumentHtml(
    //iconv('gbk','utf-8',file_get_contents($value2))
      mb_convert_encoding(file_get_contents($key2),'UTF-8','GB2312')
    );
    //var_dump($value2);exit;//pages/A20151112568595.shtml
    //$rec['url'] =pq("dl.toppic a:eq(0)","href");

    //准备要删除的，因为从phpcms后台直接录入，这里不用判读topic是否在列表数据里。先留着$rec['thumb'] =$thumbarr[$value2];先暂时注销。
    //判断topic的数据是否在列表的数据里。
